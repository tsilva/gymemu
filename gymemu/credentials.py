...
def r2_credentials():
    ...
    if any(f"{CREDENTIAL_PREFIX}_{name}" in os.environ for name in names):
        return {name: os.environ.get(f"{CREDENTIAL_PREFIX}_{name}", "").strip() for name in names}
    path = Path(os.environ.get("GYMEMU_R2_CONFIG", "~/.config/gymemu/r2.toml")).expanduser()
    if not path.exists():
