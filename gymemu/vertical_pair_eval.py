"""Partial-state vertical rollouts with explicit reference segment boundaries."""

import numpy as np
import torch


def evaluate_vertical_pair(model, data, horizons=(1, 8, 32, 128)):
    """Start at every eligible source; retain supplied nonvertical state fields.
    ...
    one_step = {
        name: dict(
            n=int(m.sum()),
            y_errors=int(wrong[m, 0].sum()),
            vy_errors=int(wrong[m, 1].sum()),
            ...
            for h in range(1, max(horizons) + 1):
                ...
                if mode != "reference" and h > 1:
                    ...
                    if mode in ("y_only", "both"):
                        inputs[:, 1] = np.floor(previous[:, 0])
                        inputs[:, 8] = np.rint((previous[:, 0] % 1) * 8)
                ...
                if h in horizons:
                    ...
                    summary[str(h)] = dict(
                        windows=len(starts),
                        y_errors=int(errors[:, 0].sum()),
                        vy_errors=int(errors[:, 1].sum()),
                        ...
                        y_mae=float(np.abs(prediction[:, 0] - target[rows, 0]).mean()),
                        y_max_error=float(np.abs(prediction[:, 0] - target[rows, 0]).max()),
    ...
    return dict(
        ...
        scope="Only selected vertical fields fed back; all others supplied; reference boundaries",
    )
