...
def main(argv=None):
    ...
    image = args.image or (
        RESOURCE_ROOT / "ops/dstack/verified-image.txt"
    ).read_text().strip()
    run_id = uuid4().hex
    ...
    task = render_task(
        recipe, image=image, run_id=run_id, compute=args.compute,
        max_duration=args.max_duration, max_price=args.max_hourly_price,
    )
    if args.dry_run:
    ...
    receipt = {
        "id": run_id, "status": "submitted", "compute": args.compute,
        "image": image, "recipe_sha256": hashlib.sha256(recipe.encode()).hexdigest(),
        "dstack_name": task["name"], "catalog_status": None,
    }
    (directory / "launch.json").write_text(json.dumps(receipt, indent=2) + "\n")
    subprocess.run(
        [
            "dstack", "apply", "-f", str(task_file), "--project", "main",
            "--no-repo", "--yes", "--detach",
        ],
