...
export const PANEL_TYPES = Object.freeze({
  ...
  controls: { module: './controls.js', minimum: { w: 3, h: 6 }, frameKinds: [] },
...
export const BUILTIN_PANEL_PRESETS = Object.freeze({
  ...
  history: { type: 'history', title: 'Input history', config: {}, placement: { x: 0, y: 14, w: 8, h: 10 } },
  controls: { type: 'controls', title: 'Playback controls', config: {}, placement: { x: 8, y: 14, w: 4, h: 10 } },
  metrics: { type: 'telemetry', title: 'Prediction error', config: { view: 'chart', metrics: ['mse', 'inference_ms', 'step'] }, placement: { x: 0, y: 24, w: 8, h: 7 } },
  model: { type: 'telemetry', title: 'Model context', config: { view: 'stats', metrics: ['history_length', 'action_history', 'action'] }, placement: { x: 8, y: 24, w: 4, h: 7 } },
});
export function panelDefinition(workspace, id) {
  const panel = workspace.panels[id];
  if (!panel || !PANEL_TYPES[panel.type]) return null;
  return { ...PANEL_TYPES[panel.type], ...panel, id, label: panel.title, frameKinds: [], enabled: panel.enabled !== false };
}
