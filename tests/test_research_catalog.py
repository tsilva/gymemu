...
def test_catalog_reads_projection_and_hides_recovery(tmp_path):
    ...
    objects = {}
    for name in (
        "best.pt", "stages/representation/best.pt", "resume.pt", "start-scene.npz",
        "metrics.jsonl",
    ):
        payload = (
            b'{"stage":"prediction","epoch":1,"train":{"loss":0.2},"validation":{"loss":0.1,"mse":0.01}}\n'
            if name == "metrics.jsonl" else name.encode()
        )
    ...
    manifest = {
        "version": 1, "bucket": "gymemu", "prefix": prefix,
        "run_id": primary_id, "objects": objects,
    }
    client.objects[f"{prefix}/manifest.json"] = json.dumps(manifest).encode()
    index = {
        "version": 1, "id": primary_id, "environment": "Breakout", "goal": "next-frame",
        "revision": "rev", "variant": None, "comparability": "cmp", "name": "run",
        "approach": "latent", "status": "complete", "best_mse": 0.01,
    ...
    assert catalog.resolve(run["checkpoints"][0]["id"]).read_bytes() in {
        b"best.pt", b"stages/representation/best.pt"
    }
